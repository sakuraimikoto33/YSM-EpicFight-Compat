...
[Configured](https://www.curseforge.com/minecraft/mc-mods/configured) は任意です。導入すると、クライアント設定を **アニメーション** と **モデル** に分類して表示します。各分類の先頭には **除外設定** があります。アニメーション側にはモデル別の持ち替え・移動・ParCool・SWEM除外をまとめ、その後に各アニメーションと自然なはしごの使用設定を表示します。モデル側には手持ち品・投射物・乗り物の除外をまとめ、その後に各モデルの使用設定を表示します。オーバーレイとキャッシュの設定はクライアント直下に残ります。表示上の分類だけを変更し、既存TOMLのキーと値は変更しません。
