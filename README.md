...
[Configured](https://www.curseforge.com/minecraft/mc-mods/configured) is optional. When installed, it groups Client settings into **Animations** and **Models**. Each group starts with an **Exclusions** folder: Animations contains model-specific item-switch, movement, ParCool, and SWEM exclusions, followed by their animation toggles and the natural-ladder toggle; Models contains held-item, projectile, and vehicle exclusions, followed by their model toggles. Overlay and cache options remain at the Client level. This is a display-only hierarchy; existing TOML keys and values are unchanged.
