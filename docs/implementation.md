...
Configured 2.2.3 or later is optional. String-targeted `@Pseudo` mixins replace only Configured's unsupported dynamic table leaves with `ConfiguredHeldItemRules`, keeping all Configured API linkage inside that optional integration boundary. The held-item replacement, item-switch animation, movement, projectile, and vehicle exclusion editors appear under the regular Client folder and add the currently selected model ID as an empty editable row; empty rows are not written. When Configured is absent, those target classes are never loaded and only the in-game settings screen is unavailable.
