...
public record MaidPreferenceQueryMessage(
        ...
        ResourceLocation offHandItem) implements CustomPacketPayload {
    public static final Type<MaidPreferenceQueryMessage> TYPE = new Type<>(
            ResourceLocation.fromNamespaceAndPath(CompatMod.MOD_ID, "maid_preference_query"));
    public static final StreamCodec<FriendlyByteBuf, MaidPreferenceQueryMessage> STREAM_CODEC =
            StreamCodec.of((output, message) -> write(message, output), MaidPreferenceQueryMessage::read);

    @Override
    public Type<MaidPreferenceQueryMessage> type() {
        return TYPE;
    }
